...
/**
 * Check if text matches any regex pattern (with safety limits)
 */
function matchesAnyPattern(text, patterns) {
  if (!patterns || !Array.isArray(patterns)) return false;
  const str = String(text || "");
  return patterns.some(pattern => {
    if (typeof pattern !== "string" || pattern.length > 200) return false; // Guard against malicious patterns
    try {
      // Limit regex complexity: no more than 2 groups, no nested quantifiers
      if (/(\([^)]*[+*][^)]*\)){2,}|\{[0-9]+,[0-9]+\}/.test(pattern)) return false;
